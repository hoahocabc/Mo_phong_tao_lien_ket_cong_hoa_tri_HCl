...
function setup() {
  ...
  titleDiv = createDiv("MÔ PHỎNG LIÊN KẾT CỘNG HOÁ TRỊ TRONG PHÂN TỬ HCl");
...
function draw() {
  ...
  // Global ambient + two moving directional lights (affect atoms/electrons)
  ambientLight(160);

  // Two moving directional lights (dynamic highlights)
  let a1 = frameCount * 0.010;
  let l1x = cos(a1) * 400;
  let l1y = sin(a1) * 240;
  directionalLight(150, 150, 150, l1x, l1y, -0.3);

  let a2 = frameCount * 0.018 + PI / 3;
  let l2x = cos(a2) * 220;
  let l2y = sin(a2) * 160;
  directionalLight(110, 110, 110, -l2x, -l2y, 0.2);
  ...
  // Draw atoms (nucleus and electrons) first — these use global lighting and stay unchanged
  ...
  // Then draw (adjusted) electron spheres on top; materials are adjusted locally so electrons/nucleus brightness unchanged
  if (isSphereVisible) {
    drawElectronSpheres();
...
// Draw electron spheres with increased brightness compared to the previous dim level,
// but still slightly less bright than electrons/nuclei so they don't overpower the scene.
function drawElectronSpheres() {
  ...
  // Dim factor: raised from 0.45 to 0.65 so spheres are brighter than before,
  // but still a little dimmer than raw electron colors.
  const dimFactor = 0.65;

  // H sphere (adjusted material)
  push();
  translate(hAtom.pos.x, hAtom.pos.y, 0);
  rotateY(hSphereRotation);
  noStroke();
  // moderate shininess so highlights are visible but not too strong
  shininess(55);
  const hr = red(hColor), hg = green(hColor), hb = blue(hColor);
  const hr_dim = hr * dimFactor, hg_dim = hg * dimFactor, hb_dim = hb * dimFactor;
  ambientMaterial(hr_dim, hg_dim, hb_dim);
  // specular increased a bit relative to previous dimmed value for subtle highlights
  specularMaterial(min(255, hr_dim + 30), min(255, hg_dim + 30), min(255, hb_dim + 30));
  sphere(hOrbitalRadius, sphereDetail, sphereDetail);
  pop();

  // Cl sphere (adjusted material)
  push();
  translate(clAtom.pos.x, clAtom.pos.y, 0);
  rotateY(clSphereRotation);
  noStroke();
  shininess(55);
  const cr = red(clColor), cg = green(clColor), cb = blue(clColor);
  const cr_dim = cr * dimFactor, cg_dim = cg * dimFactor, cb_dim = cb * dimFactor;
  ambientMaterial(cr_dim, cg_dim, cb_dim);
  specularMaterial(min(255, cr_dim + 30), min(255, cg_dim + 30), min(255, cb_dim + 30));
